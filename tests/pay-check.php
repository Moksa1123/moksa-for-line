...
use Mofoline\Pay\BlocksSupport;
use Mofoline\Pay\WooGateway;
...
$plugin_file = 'moksa-for-line/moksa-for-line.php';
...
if ( $module_on ) {
	pay_check( isset( $gateways['mofoline_pay'] ), 'the gateway is registered with WooCommerce' );
} else {
...
if ( class_exists( '\Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType' ) ) {
	...
	pay_check( 'mofoline_pay' === $method->get_name(), 'the block method carries the gateway id', $method->get_name() );

	$handles = $method->get_payment_method_script_handles();
	pay_check( in_array( 'mofoline-pay-blocks', (array) $handles, true ), 'it registers its checkout script' );
	pay_check( wp_script_is( 'mofoline-pay-blocks', 'registered' ), 'the script is registered with WordPress' );
	...
	$js = MOFOLINE_DIR . 'assets/js/pay-blocks.js';
	pay_check( is_readable( $js ), 'the checkout script ships' );

	if ( is_readable( $js ) ) {
		$source = file_get_contents( $js );
		pay_check( false !== strpos( $source, "name: 'mofoline_pay'" ), 'the script registers the same id the block method uses' );
		pay_check( false !== strpos( $source, "getSetting('mofoline_pay_data'" ), 'the script reads the data key WooCommerce derives from that id' );
	}

	// is_active() must agree with the gateway: the two checkouts show the same thing.
	$gateway = isset( $gateways['mofoline_pay'] ) ? $gateways['mofoline_pay'] : null;
...
if ( isset( $gateways['mofoline_pay'] ) ) {
	pay_check( ! $gateways['mofoline_pay']->is_available(), 'and stands down at checkout' );
}
...
foreach ( array( 'side', 'normal', 'advanced' ) as $context ) {
	foreach ( array( 'high', 'core', 'default', 'low' ) as $priority ) {
		if ( ! empty( $wp_meta_boxes[ $screen ][ $context ][ $priority ]['mofoline-pay'] ) ) {
			$found = true;
		}
	}
}
...
if ( ! empty( $any ) ) {
	...
	$module = new \Mofoline\Pay\PayModule();
	...
	$is_line_pay = 'mofoline_pay' === $order->get_payment_method();
	$expected    = array(
		__( 'This order was not paid with LINE Pay.', 'moksa-for-line' ),
		__( 'No LINE Pay record exists for this order. The payment may never have been started.', 'moksa-for-line' ),
	);
	$says_so     = false;
...
$module = isset( $module ) ? $module : new \Mofoline\Pay\PayModule();
