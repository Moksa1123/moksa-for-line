...
use Mofoline\Imagemap\ImagemapModule;
...
( static function (): void {
	...
	<div class="wrap mofoline-wrap">
		<h1><?php esc_html_e( 'Imagemaps', 'moksa-for-line' ); ?></h1>

		<p class="description">
			<?php esc_html_e( 'One banner image the customer taps. The regions are invisible to them, so draw them on the image rather than guessing coordinates.', 'moksa-for-line' ); ?>
			<?php
			printf(
				/* translators: %d: the imagemap action limit. */
				esc_html__( 'Up to %d regions. The image must be at least 1040 pixels wide; LINE is served five sizes of it from this site.', 'moksa-for-line' ),
		...
		<?php if ( ! $is_https ) : ?>
			<div class="notice notice-error">
				<p><?php esc_html_e( 'This site is served over plain HTTP. LINE only fetches imagemap images over HTTPS, so an imagemap from this site would arrive blank.', 'moksa-for-line' ); ?></p>
			</div>
		<?php endif; ?>
		...
		<div class="moksa-split moksa-split--wide">
			<div class="moksa-split__main">
				<form class="moksa-panel" data-moksa-imagemap-form>
					<h2>
						<?php esc_html_e( 'Imagemap', 'moksa-for-line' ); ?>
					...
					<div class="moksa-field-row">
						<p>
							<label for="moksa-imagemap-name"><?php esc_html_e( 'Name', 'moksa-for-line' ); ?></label>
							<input type="text" id="moksa-imagemap-name" name="name" class="widefat" required />
						</p>

						<p>
							<label for="moksa-imagemap-alt"><?php esc_html_e( 'Fallback text', 'moksa-for-line' ); ?></label>
							<input type="text" id="moksa-imagemap-alt" name="alt_text" class="widefat" maxlength="1500" required />
							<span class="description"><?php esc_html_e( 'Shown in the chat list and in push notifications, where the image cannot be drawn.', 'moksa-for-line' ); ?></span>
						</p>
					</div>

					<p>
						<label><?php esc_html_e( 'Banner image', 'moksa-for-line' ); ?></label>
						<input type="hidden" name="image_attachment_id" value="0" />
						<button type="button" class="button" data-moksa-pick-imagemap-image><?php esc_html_e( 'Choose image', 'moksa-for-line' ); ?></button>
						<span class="moksa-menu-image" data-moksa-imagemap-image-preview></span>
						<span class="description">
							<?php esc_html_e( 'At least 1040 pixels wide. Five copies are cut on save, because LINE asks for the image at five widths and refuses a URL with a file extension.', 'moksa-for-line' ); ?>
						</span>
					</p>

					<p>
						<label><?php esc_html_e( 'Tappable regions', 'moksa-for-line' ); ?></label>
						<span class="description">
							<?php esc_html_e( 'Drag on the image to draw a region. Click one to move, resize or set what it does. Coordinates are in the 1040-wide space LINE renders in.', 'moksa-for-line' ); ?>
						</span>
							...
							<span class="moksa-area-editor__canvas">
								<span class="moksa-canvas" data-moksa-canvas>
									...
									<span class="moksa-canvas__empty">
										<?php esc_html_e( 'Choose a banner image to start drawing regions.', 'moksa-for-line' ); ?>
									</span>
								</span>
						...
						<label for="moksa-imagemap-areas" class="screen-reader-text"><?php esc_html_e( 'Tappable regions as JSON', 'moksa-for-line' ); ?></label>
						...
						<button type="button" class="button-link" data-moksa-toggle-imagemap-json><?php esc_html_e( 'Edit the JSON directly', 'moksa-for-line' ); ?></button>
					</p>

					<p>
						<button type="submit" class="button button-primary"><?php esc_html_e( 'Save', 'moksa-for-line' ); ?></button>
						<button type="button" class="button" data-moksa-reset-imagemap><?php esc_html_e( 'New imagemap', 'moksa-for-line' ); ?></button>
					</p>

					<p>
						<label for="moksa-imagemap-test"><?php esc_html_e( 'Send a test to', 'moksa-for-line' ); ?></label>
						<input type="text" id="moksa-imagemap-test" class="regular-text" placeholder="U1234..." data-moksa-imagemap-test-target />
						<button type="button" class="button" data-moksa-imagemap-test><?php esc_html_e( 'Send test', 'moksa-for-line' ); ?></button>
			...
			<div class="moksa-split__side">
				<div class="moksa-panel">
					<h2><?php esc_html_e( 'Saved imagemaps', 'moksa-for-line' ); ?></h2>
					<ul class="moksa-list">
						...
						<?php foreach ( $imagemaps as $imagemap ) : ?>
							...
							<li data-imagemap='<?php echo esc_attr( (string) wp_json_encode( $row ) ); ?>'>
								...
								<?php if ( ! ImagemapImages::complete( (int) $imagemap->id ) ) : ?>
									<span class="moksa-pill moksa-pill--warn"><?php esc_html_e( 'images missing', 'moksa-for-line' ); ?></span>
								<?php endif; ?>
								<button type="button" class="button-link delete" data-moksa-delete-imagemap="<?php echo esc_attr( (string) $imagemap->id ); ?>"><?php esc_html_e( 'Delete', 'moksa-for-line' ); ?></button>
							</li>
						<?php endforeach; ?>
					</ul>
				</div>
				...
				<div class="moksa-panel">
					<h2><?php esc_html_e( 'How LINE fetches the image', 'moksa-for-line' ); ?></h2>
					<p class="description">
						<?php esc_html_e( 'LINE treats the base URL as a prefix and appends the width it wants, with no file extension. This is why the image is served from the plugin rather than straight from the media library.', 'moksa-for-line' ); ?>
					</p>
					<ul class="moksa-list">
