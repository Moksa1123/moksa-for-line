...
use Mofoline\Webhook\EventQueue;

defined( 'ABSPATH' ) || exit;

( static function (): void {
	$levels = array(
		'problems' => __( 'Warnings and errors', 'moksa-for-line' ),
		'error'    => __( 'Errors only', 'moksa-for-line' ),
		'warning'  => __( 'Warnings only', 'moksa-for-line' ),
		'info'     => __( 'Info only', 'moksa-for-line' ),
		'debug'    => __( 'Debug only', 'moksa-for-line' ),
		'all'      => __( 'Everything', 'moksa-for-line' ),
	);
	...
	$level_labels = array(
		'error'   => __( 'Error', 'moksa-for-line' ),
		'warning' => __( 'Warning', 'moksa-for-line' ),
		'info'    => __( 'Info', 'moksa-for-line' ),
		'debug'   => __( 'Debug', 'moksa-for-line' ),
	);
	...
	<div class="wrap mofoline-wrap">
		<h1><?php esc_html_e( 'Logs', 'moksa-for-line' ); ?></h1>

		<h2><?php esc_html_e( 'Recent webhook events', 'moksa-for-line' ); ?></h2>
		<table class="widefat striped">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Received', 'moksa-for-line' ); ?></th>
					<th><?php esc_html_e( 'Type', 'moksa-for-line' ); ?></th>
					<th><?php esc_html_e( 'From', 'moksa-for-line' ); ?></th>
					<th><?php esc_html_e( 'Status', 'moksa-for-line' ); ?></th>
					<th><?php esc_html_e( 'Detail', 'moksa-for-line' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php if ( empty( $events ) ) : ?>
					<tr>
						<td colspan="5">
							<?php esc_html_e( 'Nothing yet. If you have already pressed Verify in the LINE Console and this is still empty, the signature check is rejecting deliveries, which almost always means the Messaging API channel secret is wrong.', 'moksa-for-line' ); ?>
						</td>
					</tr>
				<?php endif; ?>
				<?php foreach ( $events as $event ) : ?>
					<tr>
						...
						<td>
							...
							$labels = array(
								'done'    => __( 'Handled', 'moksa-for-line' ),
								'failed'  => __( 'Failed', 'moksa-for-line' ),
								'pending' => __( 'Waiting', 'moksa-for-line' ),
							);
						...
						<td><?php echo esc_html( \Mofoline\Webhook\EventQueue::summary( $event ) ); ?></td>
					</tr>
				<?php endforeach; ?>
			</tbody>
		</table>

		<h2><?php esc_html_e( 'Plugin log', 'moksa-for-line' ); ?></h2>

		<form method="get" class="moksa-filters">
			<input type="hidden" name="page" value="mofoline-logs" />

			<label for="moksa-log-level" class="screen-reader-text"><?php esc_html_e( 'Show which entries', 'moksa-for-line' ); ?></label>
			...
			<label for="moksa-log-channel" class="screen-reader-text"><?php esc_html_e( 'Which area of the plugin', 'moksa-for-line' ); ?></label>
			<select id="moksa-log-channel" name="channel">
				<option value=""><?php esc_html_e( 'All areas', 'moksa-for-line' ); ?></option>
				<?php foreach ( $channels as $name ) : ?>
					<option value="<?php echo esc_attr( $name ); ?>" <?php selected( $channel, $name ); ?>><?php echo esc_html( $name ); ?></option>
				<?php endforeach; ?>
			</select>

			<button type="submit" class="button"><?php esc_html_e( 'Filter', 'moksa-for-line' ); ?></button>

			<span class="moksa-filters__count">
				<?php
				printf(
					/* translators: 1: rows matching the filter, 2: rows kept in total. */
					esc_html__( '%1$s of %2$s entries', 'moksa-for-line' ),
			...
			<?php if ( $grand_total > 0 ) : ?>
				<button type="button" class="button moksa-button-danger" data-moksa-clear-logs>
					<?php esc_html_e( 'Clear the log', 'moksa-for-line' ); ?>
				</button>
			<?php endif; ?>
		</form>
		...
		<p class="description">
			...
			printf(
				...
				esc_html__( 'Entries are deleted automatically after %s days, which you can change under Settings > Advanced.', 'moksa-for-line' ),
		...
		<table class="widefat striped moksa-log-table">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Time', 'moksa-for-line' ); ?></th>
					<th><?php esc_html_e( 'Level', 'moksa-for-line' ); ?></th>
					<th><?php esc_html_e( 'Area', 'moksa-for-line' ); ?></th>
					<th><?php esc_html_e( 'Message', 'moksa-for-line' ); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php if ( empty( $logs ) ) : ?>
					<tr>
						<td colspan="4">
							<?php if ( $grand_total > 0 ) : ?>
								<?php esc_html_e( 'Nothing matches this filter. Set it to Everything to see the rest.', 'moksa-for-line' ); ?>
							<?php else : ?>
								<?php esc_html_e( 'Nothing logged.', 'moksa-for-line' ); ?>
							<?php endif; ?>
						</td>
					</tr>
				<?php endif; ?>
						...
						<td>
							...
							<?php if ( '' !== $context && '[]' !== $context && '{}' !== $context ) : ?>
								...
								<details class="moksa-log-context">
									<summary><?php esc_html_e( 'Context', 'moksa-for-line' ); ?></summary>
